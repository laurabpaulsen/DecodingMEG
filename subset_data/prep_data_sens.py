...
def create_blocks(sessions, n_bins, n, animate_triggers):
    ...
    return X_block, y_block, session_inds
...
def main():
   ...
   """
    ...
    print('Check X shape', Xbin[0].shape)
    np.savez(f'data/xbin_sens.npz', Xbin[0],Xbin[1],Xbin[2],Xbin[3],Xbin[4],Xbin[5],Xbin[6], allow_pickle = True)
    np.save(f'data/ybin_sens.npy', np.array(ybin, dtype=object))
    np.save(f'data/ybin_seshinds_sens.npy', np.array(sesh_inds, dtype=object))
